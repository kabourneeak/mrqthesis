...
In Section~\ref{:convexp:remarks} we will show how to use our method even when relax some restrictions on $Q$.
...
$P_I \subseteq Q$, so we must calculate its area as part of our determination.
...
For certain values of $i$ and $j$, we will calculate the area of the subpolygon defined by the chord segment itself and the edges $e_i, e_{i+1}, \ldots, e_j$.
...
That is, that the edge sequence is of the form $e, \ldots, e_{n-1}, e_{n}, e_{1}, e_{2}, \ldots e'$.
...
Using the preprocessed subpolygon areas stored in $T_P$, the chain of edges $e_{i+1}$ to $e_{j-1}$ can be decomposed into $\log{n}$ preprocessed subpolygons whose total area is found in the tree.
