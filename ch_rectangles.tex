...
Returning the sum of the two counts, or reporting both sets of segments is straight-forward.
...
Consider the Class (i), $(p, q) \in (I, V)$ case. 
...
\begin{lemma}
Given a set $S$ of $n$ line segments, we can identify which segments belong to class (i) and which satisfy their majority property in $\BigOh{\log^4{n}}$ time using a data structure requiring $\BigOh{n\log^4{n}}$ preprocessing time and space.
\end{lemma}

Now, consider the Class (ii), $(p, q) \in (I, III)$ case. 
...
Given a set $S$ of $n$ line segments, we can identify segments belonging to class (ii) in $\BigOh{\log^3{n}}$ time using a data structure requiring $\BigOh{n\log^3{n}}$ preprocessing time and space.
\end{lemma}
...
Given a set $S$ of $n$ horizontal line segments, we can identify segments satisfying a half-plane representation of a majority expression in $\BigOh{\sqrt{n}\log{n}}$ time, using a data structure of size $\BigOh{n\log{n}}$, which can be built in $\BigOh{n\log{n}}$ time.
\end{lemma}
...
\begin{lemma}
Given a set $S$ of $n$ line segments, we can identify segments which belong to class (ii) \emph{and} which in $\BigOh{\log^3{n}}$ time using a data structure requiring $\BigOh{n\log^3{n}}$ preprocessing time and space.
...
Because they all use the same classification structure, we may save a constant factor by performing classification first, then majority expressions.
...
We've seen that the most challenging cases occur when both endpoints of a segment exist outside of the query region, but the segment nevertheless satisfies the majority property.  We have also seen how we can transform majority expressions into dual spaces which can be easily queried with a half-plane.
