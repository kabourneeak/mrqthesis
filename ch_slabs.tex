...
We will see that partitioning segments as left or right of $L_1$ is sufficient, as we can discriminate between cases 2 and 3 when testing the partial enclosure property.
...
Let $s \in S$ be a horizontal line segment defined by $(a, b, l)$.
Let $\iline{s}$ be the line through $s$, defined by the equation $y = b$.
...
This partial enclosure expression can be queried by mapping each segment $s$ to a dual-point with coordinates $(b, \rho l)$ and locating those which satisfy the half-plane
\[
