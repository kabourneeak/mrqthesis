...
Let $\iline{x}$ be the vertical line through any $x$-coordinate, and let $P_U$ and $P_L$ be the upper and lower chains of $P$, respectively. 
The sequence of lines $\iline{x_1}, \iline{x_2}, \ldots, \iline{x_n}$ passes through the leftmost vertex of $P$, the next leftmost, and so on from left to right to the rightmost vertex.
For every line $\iline{x_i}$, let $x_i = \iline{x_i} \cap P_U$ and $x_i' = \iline{x_i} \cap P_L$ be the intersections of $\iline{x_i}$ with the upper and lower chains of $P$, respectively.

We will calculate all of these intersections as we walk from left to right over both $P_U$ and $P_L$ simultaneously. 
We also keep track of $e_i$ and $e_i'$, the edges of $P_U$ and $P_L$, respectively, which are intersected by $\iline{x_i}$, and upon which reside the points $x_i$ and $x_i'$. 
...
\begin{itemize}
 ...
 \item $R_i$ is the polygon formed by the cycle on $x_i, x_{i+1}, x_{i+1}', x_i'$.
\end{itemize}
...
And we set the constant $A = \frac{1}{2} t'$.
...
To create the multi-region formula for a new critical height, we start by copying the formula for the predecessor height (the lowest $y$-value starts with 0).
To update the multi-region formula, we subtract the coefficients of $R$ for its previous phase of growth so that they no longer influence the formula, and then add in the coefficients for the new phase.
...
\begin{itemize}
 ...
 \item Its top-right: the lower of the point $x_{i+1}$ and  $h_\delta \cap \iline{x_{i+1}}$, and

 \item Its bottom-right: the higher of the point $x_{i+1}'$ and $h_\beta \cap \iline{x_{i+1}}$.
...
Let $P$ be a monotone polygon consisting of $n$ vertices, and let $0 < \rho \leq 1$ be a fixed parameter. With $\BigOh{n \log{n}}$ preprocessing time and $\BigOh{n \log{n}}$ space, we can determine if $\area{Q \cap P} \geq \rho \cdot \area{P}$ in $\BigOh{\log{n}}$ time for any axis-parallel rectangular query $Q$.
\end{corollary}
