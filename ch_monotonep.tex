...
Let $\iline{x}$ be the vertical line through $(x,0)$, and let $P_U$ and $P_L$ be the upper and lower chains of $P$, respectively. 
...
As we walk from left to right over both $P_U$ and $P_L$ simultaneously, we keep track of $e_i$ and $e_i'$, the edges of $P_U$ and $P_L$, respectively, which are intersected by $\iline{x_i}$, and upon which reside the points $x_i$ and $x_i'$. 
...
We define a sequence of regions $R_1, R_2, \dots, R_{n-1}$ as follows. 
For each $i \in \{1, 2, \ldots, n-1\}$, let $X_i$ be the vertical slab between $\iline{x_i}$ and $\iline{x_{i+1}}$, and let $R_i = X_i \cap P$, then,
...
Recall from the construction of the list of regions, $\mathcal{R}$, that if $\iline{\alpha}$ passes between $x_i$ and $x_{i+1}$, then it passes through region $R_i$, which stores the edges $e_i$ and $e_i'$ that define its top and bottom.

Given $e_i$ and $e_i'$, we define the intersection points between $\iline{\alpha}$ and the edges as $s = e_i \cap \iline{\alpha}$ and $s' = e_i' \cap \iline{\alpha}$.
